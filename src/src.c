...
void cut_index(lenArray_t *arr, size_t index) {
    ...
    memmove(&(arr->arr[index]), &(arr->arr[index + 1]),
            (arr->len - index - 1) * sizeof(int32_t));
...
int main(int argc, char **argv) {
    ...
    uintmax_t LL_ticks = LL_stop - LL_start;
    uintmax_t arr_ticks = arr_stop - arr_start;

    printf("\tLL ticks: %ju, time: %lf\n", LL_ticks,
           (double)LL_ticks / CLOCKS_PER_SEC);
    printf("\tArr ticks: %ju, time: %lf\n", arr_ticks,
           (double)arr_ticks / CLOCKS_PER_SEC);
